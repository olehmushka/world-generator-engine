...
func LoadAllTraitKinds(opts ...types.ChangeStringFunc) chan either.Either[[]string] {
	_, filename, _, _ := runtime.Caller(1)
	currDirname := tools.PreparePath(path.Dir(filename), "religion")
	dirname := currDirname + "data/trait_kinds/"
	for _, fn := range opts {
		dirname = fn(dirname)
	}
	...
	go func() {
		...
		if err != nil {
			ch <- either.Either[[]string]{Err: we.NewInternalServerError(err, fmt.Sprintf("can not read dir by dirname (dirname=%s)", dirname))}
			return
		}
